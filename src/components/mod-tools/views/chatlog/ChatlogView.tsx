    ...
    const RoomInfo = (props: { roomId: number, roomName: string }) =>
    {
        return (
            <Flex gap={ 2 } alignItems="center" justifyContent="between" className="bg-muted rounded p-1">
                <Flex gap={ 1 }>
                    <Text bold>Nombre de Sala:</Text>
                    <Text>{ props.roomName }</Text>
                </Flex>
                <Flex gap={ 1 }>
                    <Button onClick={ event => TryVisitRoom(props.roomId) }>Visitar Sala</Button>
                    <Button onClick={ event => openRoomInfo(props.roomId) }>Herramientas</Button>
                </Flex>
            </Flex>
        );
    }
            ...
            <Column fit gap={ 0 } overflow="hidden">
                <Column gap={ 2 }>
                    <Grid gap={ 1 } className="text-black fw-bold border-bottom pb-1">
                        ...
                        <Base className="g-col-3">Usuario</Base>
                        <Base className="g-col-7">Mensaje</Base>
                    </Grid>
                </Column>
