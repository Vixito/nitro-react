...
export const ModToolsRoomView: FC<ModToolsRoomViewProps> = props =>
{
    ...
    return (
        <NitroCardView className="nitro-mod-tools-room" theme="primary-slim" windowPosition={ DraggableWindowPosition.TOP_LEFT }>
            <NitroCardHeaderView headerText={ 'Información de Sala' + (name ? ': ' + name : '') } onCloseClick={ event => onCloseClick() } />
            <NitroCardContentView className="text-black">
                <Flex gap={ 2 }>
                    <Column justifyContent="center" grow gap={ 1 }>
                        <Flex alignItems="center" gap={ 2 }>
                            <Text bold align="end" className="col-7">Dueño de la sala:</Text>
                            <Text underline pointer truncate onClick={ () => (ownerId && CreateLinkEvent(`mod-tools/open-user-info/${ ownerId }`)) }>{ ownerName }</Text>
                        </Flex>
                        <Flex alignItems="center" gap={ 2 }>
                            <Text bold align="end" className="col-7">Usuarios en sala:</Text>
                            <Text>{ usersInRoom }</Text>
                        </Flex>
                        <Flex alignItems="center" gap={ 2 }>
                            <Text bold align="end" className="col-7">¿Dueño en la sala?:</Text>
                            <Text>{ ownerInRoom ? 'Sí' : 'No' }</Text>
                        </Flex>
                    </Column>
                    <Column gap={ 1 }>
                        <Button onClick={ event => TryVisitRoom(roomId) }>Visitar Sala</Button>
                        <Button onClick={ event => CreateLinkEvent(`mod-tools/open-room-chatlog/${ roomId }`) }>Historial</Button>
                    </Column>
                </Flex>
                <Column className="bg-muted rounded p-2" gap={ 1 }>
                    ...
                    <Flex alignItems="center" gap={ 1 }>
                        ...
                        <Text small>Cambiar nombre de la sala</Text>
                    </Flex>
                </Column>
                <textarea className="form-control" placeholder="Escribe un mensaje obligatorio para los usuarios en este cuadro..." value={ message } onChange={ event => setMessage(event.target.value) }></textarea>
                <Flex justifyContent="between">
                    <Button variant="danger" onClick={ event => handleClick('send_message') }>Enviar Advertencia</Button>
                    <Button onClick={ event => handleClick('alert_only') }>Enviar sólo Alerta</Button>
                </Flex>
